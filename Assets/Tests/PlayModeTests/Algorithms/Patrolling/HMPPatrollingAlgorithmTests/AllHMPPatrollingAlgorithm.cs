...
namespace Tests.PlayModeTests.Algorithms.Patrolling.HMPPatrollingAlgorithmTests
{
    ...
    public class AllHMPPatrollingAlgorithm
    {
        private static readonly AlgorithmFactory[] Cases = new[]
        {
            ...
            new AlgorithmFactory(seed => new FaultToleranceAlgorithm(seed, false, false, true), nameof(FaultToleranceAlgorithm) + ".NoMeetEarlyFixup"),
            new AlgorithmFactory(seed => new FaultToleranceAlgorithm(seed, false, false, false), nameof(FaultToleranceAlgorithm) + ".NOSENDALL"),
