...
namespace Maes.Algorithms.Patrolling.Components
{
    public class CollisionAvoidanceTargetSameVertexComponent : IComponent
    {
        ...
        /*
         * Resolving potential movement conflicts when multiple robots target the same vertex
         */
        private void CollisionAvoidanceHandling()
        {
            var targetVertex = _movementComponent.TargetVertex;
            ...
            if (receivedMessages.Any(otherRobotMessage => otherRobotMessage.TargetPosition == targetVertex.Position &&
                                                          otherRobotMessage.DistanceToVertex < distanceToVertex))
            {
