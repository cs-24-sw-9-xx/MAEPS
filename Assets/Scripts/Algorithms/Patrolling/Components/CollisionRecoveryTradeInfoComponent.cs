            ...
            while (true)
            {
                if (_controller.IsCurrentlyColliding)
                {
                    ...
                    foreach (var message in messages)
                    {
                        ...
                        for (var i = 0; i < collisionMessage.VerticesInfo.Count; i++)
                        {
                            _map.Vertices[i].LastTimeVisitedTick = Math.Max(collisionMessage.VerticesInfo[i].lastTimeVisistedTick, _map.Vertices[i].LastTimeVisitedTick);
                        }
