...
namespace Maes.Map.MapPatrollingGen
{
    public static class DelaunatorWaypointGenerator
    {
        ...
        {
            // Get all wall tiles
            var wallTiles = GetWallTiles(simulationMap);
            ...
            for (var x = 0; x < simulationMap.WidthInTiles; x++)
            {
                ...
                {
                    if (wallTiles.Contains(x, y) && IsCornerTile(new Vector2Int(x, y), wallTiles))
                    {
        ...
        /// <returns></returns>
        private static bool IsCornerTile(Vector2Int tile, Bitmap tiles)
        {
            // Horizontal grid check
            if (tiles.Contains(tile + Vector2Int.right)
            && tiles.Contains(tile + Vector2Int.left)
            && tiles.Contains(tile + Vector2Int.up)
            && tiles.Contains(tile + Vector2Int.down))
            {
                return false;
            }

            // Vertical grid check
            if (tiles.Contains(tile + Vector2Int.up)
            && tiles.Contains(tile + Vector2Int.down)
            && tiles.Contains(tile + Vector2Int.right)
            && tiles.Contains(tile + Vector2Int.left))
            {
                return false;
            }
