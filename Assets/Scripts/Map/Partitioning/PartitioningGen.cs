    ...
    public static class PartitioningGen
    {
        public static PatrollingMap MakePatrollingMapWithSpectralBisectionPartitions(SimulationMap<Tile> simulationMap, bool colorIslands, int amountOfPartitions)
        {
            ...
            foreach (var cluster in clusters)
            {
                ...
                var clustorColor = Random.ColorHSV(0f, 1f, 0.5f, 1f, 0.5f, 1f);
                var vertices = WaypointConnection.ConnectVerticesByReverseNearestNeighbor(cluster.Value.ToDictionary(p => p, p => vertexPositionsDictionary[p]), localDistanceMatrix, colorIslands, clustorColor, nextId);
