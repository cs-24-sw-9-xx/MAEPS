...
namespace Maes.Experiments.Patrolling
{
    ...
    internal class Group_CS_25_DS_10_16_Experiment : MonoBehaviour
    {
        ...
        private static readonly List<int> _seeds = new() { 123456 };
        ...
        private const float RobotFailureRate = 0.025f;
                ...
                foreach (var mapSize in _mapSizes)
                {
                    foreach (var robotCount in _robotCounts)
                    {
                        foreach (var partition in _partitionNumbers)
                        {
                            ...
                            {
                                return new RandomRedistributionWithCRAlgo(seed, 100);
                            }

                            scenarios.AddRange(ScenarioUtil.CreateScenarios(seed, "Global Redistribution", AlgorithmGlobalRedistributionWithCR, robotCount, mapSize, NumberOfCycles, RobotConstraints, partition, FaultInjection));
