    ...
    internal class GroupAMapSizeStartupTimeExperiment : MonoBehaviour
    {
        ...
        private void Start()
        {
            ...
            foreach (var seed in GroupAParameters.SeedGenerator(1))
            {
                ...
                {
                    scenarios.AddRange(GroupAExperimentHelpers.CreateScenarios(seed, "cr", (_) => new ConscientiousReactiveAlgorithm(), map => ReverseNearestNeighborGenerator.MakePatrollingMap(map, GroupAParameters.MaxDistance), 4, mapSize));
                }
            }
